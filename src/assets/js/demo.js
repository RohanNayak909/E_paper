...
//     var files = document.getElementById("pdf-file").files;
...
async function openfile(evt) {
    ...
    for (var i = 0; i < files.length; i++) {
        await multipleFile(files[i]);
    }
}

function multipleFile(files) {
...
function listFilesOnScreen() {
    ...
    if (window.arrayOfPdf.length > 0) {
        ...
        $.each(window.arrayOfPdf, function (i, v) {
            $('.list-files').append(` 
           <div id="pdf-${i}" style="width:96px;margin:0px 10px;word-break: break-all;" class="pdf-file">
           <div onclick="removePdf(${i})" class="btn-remove" style="cursor:pointer">x</div>
...
async function joinPdf() {
    ...
    var data = download(pdfBytes, "Merged-PDF-" + `${day}${month + 1}${year}-${hour}${min}` + ".pdf", "application/pdf");
    return pdfBytes;
}
